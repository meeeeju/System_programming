...
void main(){
    ...
    while(1){
        if(kbhit()) do{key=kgetch();} while(key==224); //키 입력받음
        sleep(speed);
...
void title(void){
    ...
    while (kbhit()) kgetch(); //버퍼에 있는 키값을 버림 
    ...
    gotoxy(MAP_X+(MAP_WIDTH/2)-7,MAP_Y+17," System_Programming_team4");    
...
/*void draw_map(){ //맵 테두리 그리는 함수 
    ...
    for(i=0;i<MAP_WIDTH;i++){
        gotoxy(MAP_X+i,MAP_Y,"ㅁ");
    }
    for(i=MAP_Y+1;i<MAP_Y+MAP_HEIGHT-1;i++){
        gotoxy(MAP_X,i,"ㅁ");
        gotoxy(MAP_X+MAP_WIDTH-1,i,"ㅁ");
    }
    for(i=0;i<MAP_WIDTH;i++){
        gotoxy(MAP_X+i,MAP_Y+MAP_HEIGHT-1,"ㅁ");
    }
}*/

void draw_map()//This can be done in a better way... FIX ME!!!! Also i think it doesn't work properly in ubuntu <- Fixed
{
    //int i;
    int x = 1, y = 1;
    clear(); //clear the console

    move(x,y); //Top left corner
...
void stop(void){ // p키를 눌렀을 경우 게임을 일시 정지 
    while(1){
        if(key==PAUSE){
